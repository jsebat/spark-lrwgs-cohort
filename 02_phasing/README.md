# 02_phasing — trio-resolved and physical phasing for long-read families
...
recombination maps, phase-aware QC of a cohort — and it is also the first stage of `05_denovo`,
which was its first consumer and where this code lived until it was extracted.

> **Numbering.** `01_qc` already exists, so `02_phasing` shares the 01 slot. The intent (JS) is that
...
> `05_denovo/workflow/Snakefile`'s comment.

**Provenance.** This was the `M1` sub-module of `05_denovo`. The extraction was a move with **no
...
(per-haplotype depth) and P19 (duos excluded) are recorded in `05_denovo/DESIGN.md`; decisions taken
...
`io/vcf.py` is a **copy** of `05_denovo/src/phase_dnm/io/vcf.py`, not a move: `candidates.py`,
...
# on a machine that already has 05_denovo configured, this reproduces the setup exactly:
#   ln -s ../../05_denovo/config/phase_dnm.env config/phasing.env
```
...
`PHASE_DNM_PYTHON`, so an environment set up for `05_denovo` works unchanged.

Tests: `python -m pytest` (32 tests, a ~300 kb synthetic trio, no cluster needed).

## 5. The audit — what it asserts

`trio-phase audit` exists for the reason `05_denovo/src/phase_dnm/audit.py` exists: every expensive
...
`05_denovo` (phase-aware de novo calling) reads the tables **by path** under `$PHASE_DIR` — its
...
documented in §3, and `05_denovo`'s own `LabelTables` (in `evidence/hapmatrix.py`) is the reader.
`05_denovo/workflow/Snakefile` lists `$PHASE_DIR/<family>/m1_xoreads.status` as an input of
...
1. **Nothing in `evidence/hapmatrix.py` moved.** The only phasing-related thing there is `LabelTables`,
   ...
   `05_denovo` into this module that the split is meant to avoid. It stays where it is.
2. **`PLAN_MODULE1.md` stayed in `05_denovo`.** Despite the name it is a running log covering M2–M4
...
4. **`config/thresholds.yaml` is a verbatim copy, version string and all.** The `version` is written
   ...
   `thresholds_version` `0.1.0` while `05_denovo` has since moved to `0.3.0` for M2/M3 reasons — a
   pre-existing drift that this split prevents recurring.)*
5. **`PAR_GRCH38` was restated, not imported.** `05_denovo/src/phase_dnm/features/extract.py` used to
