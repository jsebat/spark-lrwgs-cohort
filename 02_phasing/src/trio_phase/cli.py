...
PLANNED = {
    "haplotag": "export to BAM (optional, IGV); labels are the orientation/transmission tables",
}
...
def cmd_transmission(a: argparse.Namespace) -> int:
    ...
    if not os.path.exists(orient_path):
        sys.exit("orientation table not found (%s); run `trio-phase orient` first" % orient_path)
...
def cmd_phase_qc(a: argparse.Namespace) -> int:
    ...
    if not rows:
        sys.exit("no children with phasing outputs under %s" % a.phase_dir)
...
def cmd_audit(a: argparse.Namespace) -> int:
    """Assert that what the configuration declares is what the phasing tables contain. Non-zero on any FAIL."""
    from . import audit as AU
    return AU.run(a.phase_dir, a.manifest, hapdepth_dir=a.hapdepth_dir, family=a.family,
                  strict_across_block=a.strict_across_block,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="trio-phase", description=__doc__)
    ap.add_argument("--version", action="version", version=__version__)
    sub = ap.add_subparsers(dest="cmd", required=True)

    o = sub.add_parser("orient", help="orient the child's phase blocks to parent of origin")
    _trio_args(o)
    o.set_defaults(func=cmd_orient)

    t = sub.add_parser("transmission", help="transmitted/untransmitted map per parent + change points")
    _trio_args(t)
    ...
    h = sub.add_parser("hapdepth", help="per-haplotype depth in fixed bins from one haplotagged BAM (pysam)")
    ...
    x = sub.add_parser("xo-reads", help="classify change points as CROSSOVER / SWITCH_ERROR from parent reads (pysam)")
    ...
    qc = sub.add_parser("phase-qc", help="per-child phase_qc.json and the cohort QC table with gates")
    ...
    au = sub.add_parser("audit", help="assert that what the config declares is what the phasing tables contain")
    au.add_argument("--phase-dir", required=True, help="$PHASE_DIR holding <FAMILY>/ sub-directories")
    au.add_argument("--manifest", required=True, help="cohort manifest TSV; supplies the expected complete trios")
    au.add_argument("--hapdepth-dir", help="$PHASE_DIR/hapdepth (optional; enables the hapdepth coverage check)")
    au.add_argument("--family", help="one family only (default: every family in the manifest)")
    au.add_argument("--strict-across-block", action="store_true",
                    help="treat orientation overlaps between DIFFERENT phase blocks as a FAIL rather than a WARN "
                         "(nested HiPhase blocks make a handful of these normal; see README)")
