...
- **Q10** one Apptainer image — `ASSUMED` yes (`singularitypro/4.1.2`); the WDL's own `pb_wdl_base` image (in the miniwdl singularity cache) already carries samtools/bcftools and is what `06_clinical/sv_table.py` uses — reuse it for the read-level steps until the module image exists.
...
New directory `05_denovo/`. Reuses, not duplicates: `12_methylation/pofo_local.py` (nearest-informative-site rephasing with phase-switch flag → `local_rephase_agreement`; fallback parent-of-origin for untagged reads); `06_clinical/sv_read_review.sh` (junction detection by `SA` tag / CIGAR `D ≥ MINBIG`, per-haplotype depth inside vs flanks, het-SNV persistence → SV adapter and two SV features); `03_tiering/tr_outliers.py` (TRGT `AL`/`SD`, spanning-read QC, margin rule → TR adapter and TR candidate generator). GraphTyper's three-way outcome (METHODS §5) is an orthogonal *label* for exonic SVs, never a feature.
...
*Tradeoffs.* WhatsHap `--ped` (PedMEC) would re-phase all three samples jointly and give longer blocks, but is slow at 22× genome-wide, SNV-only, and would create a second phasing disagreeing with the `HP` tags already on 2.8 M reads/sample. Orientation is one pass over the VCF, keeps one phasing, and its failure mode is measurable (`AMBIGUOUS` rate, `n_mendel_err`). The 18 imprinting-control regions already phased to parent of origin by `12_methylation` are free ground truth for orientation. WhatsHap `--ped` is an M1 QC comparison on 2–3 trios, chr20–22 only.
...
- **SNV/indel — H1 slivar raw** (`01_qc/family_downstream.sb`): child het, both parents hom-ref, GQ ≥ 20 and DP ≥ 10 in all three, child AB 0.25–0.75, parental AD[1] = 0; knob = GQ. **H2 hiconf** = H1 ∩ outside the segdup/repeat mask ∩ gnomAD AF < 0.001. **H3 cohort** = H2 ∩ PON leave-one-family-out ∩ not sib-shared (`04_panel/apply_pon_loo.sb`, `06_clinical/denovo_tier.py`).
...
- **TR — H1 per-family** (`family_downstream.sb`): child's longest allele ≥ longest parental + 50 bp and ≥ 1.5× (any uncalled member → not evaluable); knob = gain_bp. **H2 cohort** (`03_tiering/tr_outliers.py`): SD ≥ 5 on every allele, child allele > max(parents) + max(6 bp, k × shortest motif) and > founder maximum, founder reference leave-one-family-out.
...
pipeline's LOO, `04_panel/apply_pon_loo.sb`); for a synthetic trio both the child's real parents (who carry the
