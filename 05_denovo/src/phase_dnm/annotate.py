...
def tr_founder_alleles(cohort_trgt_vcf: str, sites_bed: str, founder_ids: List[str], bcftools: str, work: str,
    ...
    The founder reference of `03_tiering/tr_outliers.py`, ported: an allele counts only when its spanning-read depth
...
def _pct(sorted_vals: List[int], q: float) -> Optional[float]:
    """Nearest-rank percentile, as in 03_tiering/tr_outliers.py."""
    if not sorted_vals:
        return None
    import math
