...
*Clarification (JS asked 2026-09-12 whether "transmitted" is labelled in the swap):* **it is not, and cannot be.** A synthetic trio has no meiosis connecting child and surrogate parents, so "the parental haplotype the child inherited" does not exist for positives; any transmission-dependent quantity would be defined for negatives only and the classifier would learn the definedness, not the biology. The transmitted-haplotype test therefore exists only in real trios (M1 transmission map) and only in the post-RF layer. What *is* well defined in a synthetic trio is the **child's parent of origin**: M1 orients the child's blocks once, with the real parents, and the label is a property of the child's genome that survives any re-pairing. Hence a *parent-of-origin-selected, haplotype-symmetric* parental feature — alt reads on either haplotype of the parent the child's alt haplotype came from (`po_parent_max_alt_any_hap`, `po_parent_min_hap_dp`) — is `rf_safe: true` and more targeted than the max over both parents. The parent-of-origin label itself stays out of the RF: inherited positives are ~50:50 paternal/maternal while true DNMs inside the negative class are ~80 % paternal, so the label would encode negative-set contamination, not evidence.
